...
(function (factory) {
  ...
  L.Control.Position = L.Control.extend({
    ...
    placeCrosshair: function (destination) {
      ...
      let marker = L.marker(
        L.latLng(destination.globalY + 0.5, destination.globalX + 0.5),
        {
          icon: icon,
        }
      );
    ...
    validateCoordinate: function (destination) {
      return (
        destination &&
        destination.plane < 4 &&
        this._map.options.maxBounds.contains(
          L.latLng(destination.globalY, destination.globalX)
        )
      );
    },
    ...
    redrawRect: function () {
      let position = this._map.containerPointToLatLng(
        this._containerPointCache
      );
      this.globalX = parseInt(position.lng);
      this.globalY = parseInt(position.lat);
      let jCoord = this.createString(
        this.convert(this._map._plane, this.globalX, this.globalY)
      );
  ...
  L.Map.addInitHook(function () {
    if (this.options.positionControl) {
      this.positionControl = new L.Control.Position(
